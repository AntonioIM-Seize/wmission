...
import { updateDevotionAction } from '@/app/(public)/devotion/[id]/actions';
...
const defaultValues: DevotionUpdateValues = {
  id: '',
  title: '',
  scriptureRef: '',
  scriptureText: '',
...
type DevotionEditFormProps = {
  initialValues: DevotionUpdateValues;
};

export function DevotionEditForm({ initialValues }: DevotionEditFormProps) {
  const [serverError, setServerError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  const form = useForm<DevotionUpdateValues>({
    resolver: zodResolver(devotionUpdateSchema),
    defaultValues: {
      ...defaultValues,
      ...initialValues,
    },
  ...
  const handleSubmit = form.handleSubmit((values) => {
    ...
    startTransition(async () => {
      const result = await updateDevotionAction(values);

      if (result && result.status === 'error') {
        if (result.fieldErrors) {
          Object.entries(result.fieldErrors).forEach(([key, errors]) => {
            if (errors?.length) {
              form.setError(key as keyof DevotionUpdateValues, {
                type: 'server',
                message: errors[0],
              });
            }
